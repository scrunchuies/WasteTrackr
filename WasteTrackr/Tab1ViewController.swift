...
class Tab1ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func observeItems() {
        let collectionId = collectionID(forSuffix: "FOH")  // Adjust as needed for dynamic collection names
    ...
    @objc func addNewItem() {
        let db = Firestore.firestore()
        let collectionId = collectionID(forSuffix: "FOH")  // Use the dynamically determined collection ID
        let newItem = [
            "name": "New Item",
            "count": 1,
...
extension Tab1ViewController: EditableCellDelegate {
